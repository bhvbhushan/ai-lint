...
describe("loadConfig", () => {
  test("loads valid .ai-lint.yml", () => {
    const config = loadConfig(join(FIXTURES_DIR, "valid.ai-lint.yml"));
  ...
  test("returns defaults when no config file exists", () => {
    const config = loadConfig(join(FIXTURES_DIR, "nonexistent.ai-lint.yml"));
  ...
  test("returns defaults for empty YAML file", () => {
    const config = loadConfig(join(FIXTURES_DIR, "empty.ai-lint.yml"));
  ...
  test("throws on invalid YAML syntax", () => {
    expect(() =>
      loadConfig(join(FIXTURES_DIR, "bad-yaml.ai-lint.yml")),
    ).toThrow(/Invalid YAML/);
  });

  test("throws on invalid config structure", () => {
    expect(() =>
      loadConfig(join(FIXTURES_DIR, "invalid.ai-lint.yml")),
    ).toThrow(/Invalid config/);
  });

  test("merges partial config with defaults", () => {
    const config = loadConfig(join(FIXTURES_DIR, "partial.ai-lint.yml"));
  ...
  test("handles permission denied gracefully", () => {
    ...
    const config = loadConfig("/nonexistent/path/.ai-lint.yml");
